...
Сценарий: Скриншоты
	...
	* Продолжим. В данном поле указываются имена процессов через точку с запятой, для которых надо снять скриншот.
		И я делаю подсветку нескольких элементов VA с их заголовком "Список процессов" UI Automation
			| 'Имя'                         |
			| 'ПроцессыДляСнятияСкриншотов' |
	* Имя процесса можно указывать не полностью, используя символ *.
	* В данной настройке не нужно указывать имена процессов 1С, а нужно указывать имена процессов приложений, которые дополнительно запускаются во время тестирования.
		И я делаю подсветку нескольких элементов VA с их заголовком "Список процессов" UI Automation
			| 'Имя'                         |
			| 'ПроцессыДляСнятияСкриншотов' |
	* Например имя процесса эмулятора мобильного приложения. Или имя процесса текстового редактора word.
	* Настройка имеет смысл только скриншоты снимаются с помощью внешней компоненты VanessaExt.
		И я делаю подсветку нескольких элементов VA с их заголовком "Делать скриншот с помощью компоненты" UI Automation
			| 'Имя'                                        |
			| 'ИспользоватьВнешнююКомпонентуДляСкриншотов' |

	* Продолжим. В данном поле выбирается как именно будут сниматься скриншоты с помощью внешней компоненты с окон 1С.		
	...
	* Продолжим. В данном поле указывается команда снятия скриншотов с помощью дополнительной утилиты.
		И я делаю подсветку нескольких элементов VA с их заголовком "Снятие скришотов внешней утилитой" UI Automation
			| 'Имя'                                      |
			| 'КомандаСделатьСкриншот' |
	* Данная настройка имеет смысл только если скриншоты будут сниматься с помощью дополнительной утилиты, а не с помощью внешней компоненты VanessaExt.
